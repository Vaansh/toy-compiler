    ...
    def handle_digit(
        ...
        if c == ".":
            ...
            if len(res_fraction) != 0:
                ...
                if lexeme[-1] == ".":
                    self.backup_character()
                    return self.create_token("intlit", lexeme[:-1], coordinates)

                token = self.create_token("floatlit", lexeme, coordinates)
            else:
                token = self.create_token("intlit", lexeme, coordinates)
        else:
            self.backup_character()
            token = self.create_token("intlit", lexeme, coordinates)
